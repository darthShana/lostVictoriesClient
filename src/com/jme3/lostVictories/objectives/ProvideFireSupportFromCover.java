...
class ProvideFireSupportFromCover extends Objective<CadetCorporal> implements MinimapPresentable{
    ...
    public static Vector3f findAttackPassition(Vector3f enemyPosition, GameCharacterNode character, Node rootNode, Set<GameStructureNode> structuresInRange) {
        ...
        for(GameStructureNode node:structuresInRange){
            List<Vector3f[]> poss = node.getCoverPossition(enemyPosition);
            if(!poss.isEmpty()){
                if(character.hasProjectileWeapon()){
                    for(Vector3f[] pos: poss){
                        if(coverPossition==null || (character.getLocalTranslation().distance(pos[0])<character.getLocalTranslation().distance(coverPossition) && pos[0].distance(enemyPosition)<character.getMaxRange())){
                            coverPossition = pos[0];
                        }
                    }                    
                } else{
                    Vector3f[] pos = node.getAttackPossition(character, enemyPosition, rootNode);
                    if(pos!=null){
                        if(coverPossition==null || (character.getLocalTranslation().distance(pos[0])<character.getLocalTranslation().distance(coverPossition) && pos[0].distance(enemyPosition)<character.getMaxRange())){
                            coverPossition = pos[1];
                        }
                    }
                }
            }
