...
public class CharacterAction {
    ...
    Vector3f traversePath(GameCharacterNode c, float f, Vector3f currentPos, Node rootNode) {
        ...
        if(!waypoints.isEmpty()){
            if(WorldMap.isClose(currentPos, waypoints.get(waypoints.size()-1))){
                ...
                if(!waypoints.isEmpty() && WorldMap.isClose(currentPos, waypoints.get(0))){
                    waypoints.remove(0);
                    if(!waypoints.isEmpty()){
                        final Float height = WorldMap.get().getTerrainHeight(waypoints.get(0), rootNode);
                        waypoints.get(0).y = (height!=null)?height:waypoints.get(0).y;
                    }
                }
