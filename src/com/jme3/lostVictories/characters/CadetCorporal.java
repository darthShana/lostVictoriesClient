...
import com.jme3.lostVictories.objectives.RemanAbandonedVehicles;
...
import com.jme3.lostVictories.objectives.CollectUnusedEquipment;
...
public class CadetCorporal extends Soldier implements CommandingOfficer{
    ...
    public CadetCorporal(UUID id, Node player, Country country, CommandingOfficer commandingOfficer, Vector3f position, Vector3f rotation, Node rootNode, BulletAppState bulletAppState, CharcterParticleEmitter characterParticleEmitters, ParticleManager particleManager, NavigationProvider navMeshPathfinder, AssetManager assetManager, BlenderModel model, BehaviorControler behaviorControler, Camera camera) {
        ...
        behaviorControler.addObjective(new RemanAbandonedVehicles());
        behaviorControler.addObjective(new CollectUnusedEquipment());
    ...
    public void attack(Vector3f target, GameCharacterNode issuingCharacter) {
        ...
        Set<Vector3f> targets = new HashSet<Vector3f>();
        targets.add(target);
        behaviorControler.addObjective(new AttackBoggies(targets, rootNode));
    }

    @Override
    public void collect(Pickable pickable, GameCharacterNode issuingCharacter) {
        final Set<Commandable> charactersUnderCommand = getCharactersUnderCommand();
    ...
    public Set<Commandable> getCharactersUnderCommand() {
        return new HashSet<Commandable>(charactersToCommand);
    }
    ...
    public boolean hasProjectileWeapon() {
        if(super.hasProjectileWeapon()){
            return true;
        }
        for(Commandable n:charactersToCommand){
            if(n instanceof GameCharacterNode){
                if(((GameCharacterNode)n).hasProjectileWeapon()){
                    return true;
                }
            }
        }
        ...
        for(Commandable n: getCharactersUnderCommand()){
            if(n instanceof GameCharacterNode){
                if(((GameCharacterNode)n).getChild("subselection")==null){
                    ((GameCharacterNode)n).attachChild(g.clone(false));
                }
            }
        }
