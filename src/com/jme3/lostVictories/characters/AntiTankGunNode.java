...
public class AntiTankGunNode extends GameVehicleNode{
    ...
    public Vector3f getPositionToTarget(GameCharacterNode targetedBy) {
        ...
        return getLocalTranslation().add(new Vector3f(0f, 3.0f, 0f));
    }

    @Override
    public void onAnimCycleDone(AnimControl control, AnimChannel channel, String animName) {
        gunnerModel.transitionFireingSequence(operatorChannel.get(country), muzzelFlash);
    ...
    public boolean doWeaponDamage(CanInteractWith victim, CollisionResult result) {
        if(victim instanceof GameVehicleNode){
            return ((GameVehicleNode)victim).takeMissile(result, this);
        }else{
