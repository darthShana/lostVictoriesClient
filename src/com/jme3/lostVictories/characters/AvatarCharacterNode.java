...
public class AvatarCharacterNode extends GameCharacterNode<BetterSoldierControl> implements CommandingOfficer{
    ...
    private Set<Commandable> charactersUnderCommand = new HashSet<Commandable>();
    ...
    public void travel(Vector3f contactPoint, GameCharacterNode issuingCharacter) {
        ...
        try{
            final Optional<List<Vector3f>> computePath = pathFinder.computePath(.8f, getLocalTranslation(), contactPoint);
            if(computePath.isPresent()){
                characterAction.travelPath(computePath.get());
            }
        }catch(Throwable e){
            e.printStackTrace();
        }
    ...
    protected void fireInDirection(Vector3f direction) {
        if(canPlayMoveAnimation(channel.getAnimationName()) && !isDead){
            playerControl.setViewDirection(direction);
            channel.setAnim(model.getWeapon().getName()+"_shootAction", LoopMode.DontLoop, 1.5f);
            fire(new Vector3f(getLocalTranslation().x, getLocalTranslation().y+Soldier.SHOOTING_HEIGHT, getLocalTranslation().z), direction);
        }
    ...
    public EnemyActivityReport getEnemyActivity() {
        ...
        for(GameCharacterNode other:WorldMap.get().getCharactersInDirection(this, getPlayerDirection(), getWeapon().getMaxRange(), false)){  
    ...
    public CharacterMessage toMessage() {
        ...
        toMessage.addAction(characterAction.toMessage());
    ...
    void clearBoardedVecicleControl() {
        if(boaredVehicleControl!=null){
