...
public class MoveAction implements AIAction<Soldier> {
    ...
    public MoveAction(Soldier c, Vector3f destination, Vector3f facePoint) {
    ...
    public boolean doAction(Soldier c, Node rootNode, GameAnimChannel channel, float tpf) {
        ...
        if(!waypoints.isEmpty() && WorldMap.isClose(currentPos, waypoints.get(0))){
            ...
            if(!waypoints.isEmpty()){
                final Float terrainHeight = WorldMap.get().getTerrainHeight(waypoints.get(0), rootNode);
                waypoints.get(0).y = terrainHeight!=null?terrainHeight:waypoints.get(0).y;
            }
        }
