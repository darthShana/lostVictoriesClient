...
public class CaptureTownTest {
    ...
    public void testCalculateGameSector() {
        CaptureTown captureTown = new CaptureTown(mock(HeerCaptain.class), new Node());
        final WorldMap worldMap = mock(WorldMap.class);
        Set<GameHouseNode> houses = new HashSet<GameHouseNode>();
        for(HouseMessage h: getAllHouses()){
    ...
    public void testMerge(){
        GameHouseNode h1 = mock(GameHouseNode.class);
        GameHouseNode h2 = mock(GameHouseNode.class);
    ...
    @Test
    public void testAssignDifferentSectorsToDiffLieutenants(){
        final HeerCaptain colonel = mock(HeerCaptain.class);
        when(colonel.getLocalTranslation()).thenReturn(Vector3f.ZERO);
        
        Set<Commandable> units = new HashSet<Commandable>();
        units.add(AICharacterNodeTest.createLieutenant(colonel));
        units.add(AICharacterNodeTest.createLieutenant(colonel));
        units.add(AICharacterNodeTest.createLieutenant(colonel));
        units.add(AICharacterNodeTest.createLieutenant(colonel));
        ...
        assertEquals(4, captureTown.sectorAssignments.keySet().size());
        
    }

    private GameSector createSector(final Rectangle rectangle) {
        final GameSector gameSector = new GameSector(rectangle);
        final GameHouseNode house = mock(GameHouseNode.class);
        when(house.getLocalTranslation()).thenReturn(new Vector3f(rectangle.x, 100, rectangle.y));
        gameSector.add(house);
        return gameSector;
    }
    ...
    private GameHouseNode convertToHouse(HouseMessage house) {
        Node n = new Node();
        final Vector3f l = house.getLocalTranslation();
        n.setLocalTranslation(l.x, l.y, l.z);
